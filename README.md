...
* **個人化背景系統**
  支援預設桌布、純色背景與自訂背景圖片，使用者可從相簿選擇圖片並透過全螢幕裁切設定桌布。並於自訂背景啟用時為導覽列、標籤列、區塊標題及進度文字加入毛玻璃底板與對比度增強，確保各種亮度背景下的可讀性。
* **App 圖示切換**
  提供多種替代 App 圖示（藍色、深色等），使用者可在設定中自由切換。透過 `CFBundleAlternateIcons` 機制實作，搭配 Asset Catalog 預覽圖與 bundle 內建 PNG 資源。
* **通知設定（功能尚未完成）**
  獨立通知設定頁面，可分別控制里程到期提醒、可兌換通知等各項通知開關。
* **首次啟動引導**
...
| **CFBundleAlternateIcons** | App 圖示動態切換 |
| **UserNotifications** | 通知設定與管理 |
